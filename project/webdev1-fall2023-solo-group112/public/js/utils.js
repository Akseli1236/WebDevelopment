 ...
 * a document).
 *
 * @returns {string} Returns a string that is random id for new users
 */
const generateId = () => {
...
/**
 * Adds product to session storage
 * 
 * @param {any} productId ProductId
 * @returns {Integer} Returns 1 if new item count + 1 if not new
 */
const addProductToCart = productId => {
  ...
  //    key: productId
  //    data: productCount + 1
    const newCount = productCount === undefined ? 1 : productCount + 1;
...
/**
 * Decreases product count from session storage
 * 
 * @param {any} productId ProductId
 * @returns {Integer} Returns the new count of products in cart
 */
 ...
 * 
 * @param {any} productId ProductId
 * @returns {Integer} Returns the product count or undefined if nothing in cart
 */
const getProductCountFromCart = productId => {
  ...
  //     value of the session storage item)
    ...
    return productCount ? parseInt(productCount) : undefined;

};
/**
 *  Gets all products from cart
 * 
 * @returns {Array} Returns arrau of products
 */
