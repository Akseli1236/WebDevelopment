 ...
 * 
 * @returns {string} Retursn a random id for new users
 */
const generateId = () => {
    //let id;
 ...
 * @returns {object|undefined} returns user or undefined if no user
 */
const getUser = (email, password) => {
    const sendUser = data.users.find(user => user.email === email && user.password === password);
 ...
 * @returns {object|undefined} Return user if id found undefined if not
 */
const getUserById = userId => {
    // TODO: 8.4 Find user by user id
    const sendUser = data.users.find(user => user._id === userId);
    return sendUser && { ...sendUser };
};
 ...
 *
 * @param {object} user User that we want to save
